...
* @{
*/
#define USER_TIME_MS 2000                                                  ///< Timing time in milliseconds
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagspray)
    {
        info->event[info->num] = EVENT_spray;
        info->num++;
        dataPoints->valuespray = gizStandardDecompressionValue(spray_BYTEOFFSET,spray_BITOFFSET,spray_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagsun_louver)
    {
        info->event[info->num] = EVENT_sun_louver;
        info->num++;
        dataPoints->valuesun_louver = gizStandardDecompressionValue(sun_louver_BYTEOFFSET,sun_louver_BITOFFSET,sun_louver_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagexhaust_air)
    {
        info->event[info->num] = EVENT_exhaust_air;
        info->num++;
        dataPoints->valueexhaust_air = gizStandardDecompressionValue(exhaust_air_BYTEOFFSET,exhaust_air_BITOFFSET,exhaust_air_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagclean)
    {
        info->event[info->num] = EVENT_clean;
        info->num++;
        dataPoints->valueclean = gizStandardDecompressionValue(clean_BYTEOFFSET,clean_BITOFFSET,clean_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuespray != cur->valuespray)
    {
        GIZWITS_LOG("valuespray Changed\n");
        ret = 1;
    }
    if(last->valuesun_louver != cur->valuesun_louver)
    {
        GIZWITS_LOG("valuesun_louver Changed\n");
        ret = 1;
    }
    if(last->valueexhaust_air != cur->valueexhaust_air)
    {
        GIZWITS_LOG("valueexhaust_air Changed\n");
        ret = 1;
    }
    if(last->valueclean != cur->valueclean)
    {
        GIZWITS_LOG("valueclean Changed\n");
        ret = 1;
    }
    ...
    if(last->valuenh3_ppm != cur->valuenh3_ppm)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuenh3_ppm Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(spray_BYTEOFFSET,spray_BITOFFSET,spray_LEN,(uint8_t *)devStatusPtr,dataPoints->valuespray);
    gizStandardCompressValue(sun_louver_BYTEOFFSET,sun_louver_BITOFFSET,sun_louver_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesun_louver);
    gizStandardCompressValue(exhaust_air_BYTEOFFSET,exhaust_air_BITOFFSET,exhaust_air_LEN,(uint8_t *)devStatusPtr,dataPoints->valueexhaust_air);
    gizStandardCompressValue(clean_BYTEOFFSET,clean_BITOFFSET,clean_LEN,(uint8_t *)devStatusPtr,dataPoints->valueclean);
    ...
    devStatusPtr->valuenh3_ppm = exchangeBytes(gizY2X(nh3_ppm_RATIO,  nh3_ppm_ADDITION, dataPoints->valuenh3_ppm)); 

    devStatusPtr->valuelight = exchangeWord(gizY2X(light_RATIO,  light_ADDITION, dataPoints->valuelight)); 
