...
unsigned char dataBuffer[42],dataBuffer1[42];
...
uart0_rx_intr_handler(void *para)
{
    ...
    */
    ...
    else if(UART_RXFIFO_FULL_INT_ST == (READ_PERI_REG(UART_INT_ST(uart_no)) & UART_RXFIFO_FULL_INT_ST))
    {
        ...
        for(buf_idx=0;buf_idx<fifo_len;buf_idx++)//鎺ユ敹涓插彛鏁版嵁
        {
    ...
    else if(UART_RXFIFO_TOUT_INT_ST == (READ_PERI_REG(UART_INT_ST(uart_no)) & UART_RXFIFO_TOUT_INT_ST))
    {
        ...
        for(buf_idx=0;buf_idx<fifo_len;buf_idx++)//uart0接收数据
        {
            temp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
            dataBuffer1[buf_idx]=temp;
        }
        for(fifo_len=0;fifo_len<42;fifo_len++){
        	if(dataBuffer[fifo_len]!=dataBuffer1[fifo_len]){
        		break;
        	}
        }
        if(fifo_len==42){
        	...
        	for(fifo_len=0;fifo_len<42;fifo_len++){
        		dataBuffer[fifo_len]=dataBuffer1[fifo_len];
        	}
        	updataFlag=1;
        }
        // pos_current += fifo_len;
...
uart_recvTask(os_event_t *events)
{
    if(events->sig == 0){
        ...
        for(idx=0;idx<fifo_len;idx++) {
            d_tmp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;//鎺ユ敹涓插彛鏁版嵁
